...
* [**FaNAT-client**](#fanat-client)
  ...
  * [Exemples](#exemples-1)
	...
	* [Reverse SSH Tunneling](#reverse-ssh-tunneling)
        * [Reverse SSH Tunneling](#reverse-ssh-tunneling)
        * [Use another available server1](#use-another-available-server)
...
### Functions  
  ...
  - отдельно, использование командной строки на управляемом устройстве. [Use Command line](#use-command-line);
  - возможность запустить ssh-клиент с обратным туннелем и получить доступ к устройству по ssh-протоколу. [Reverse SSH Tunneling](#reverse-ssh-tunneling);
...
**Позволяет запустить ssh-клиент с обратным тунелем и полуть доступ к устройству(_dev_target_).**
#### &ensp;&nbsp;Server requirements
- сервер _**152.201.19.144**_ должен иметь **белый IP**;
- на сервер _**152.201.19.144**_ должен быть доступен порт _**6744**_;
  
<br>Используя FaNATClient - Menu - Comand line - отправляем команду:
~~~
#sshpass -p password ssh -o StrictHostKeyChecking=accept-new user_server@152.201.19.144 -R 6744:localhost:22
~~~
или с ключем
~~~
#ssh  -i /home/user/folderKey/ssh-key-2020-11-09.key user_server@152.201.19.144 -R 6744:localhost:22
~~~
