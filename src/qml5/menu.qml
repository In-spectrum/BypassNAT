    ...
    onHeightChanged:
    {
        if(id_loadMenuItem.visible)
        {
            if(id_loadMenuItem.m_iPage == 6)
                id_loadMenuItem.height = id_loadMenuItem.parent.height - id_loadMenuItem.y;
            else
            {
                if(height - id_colMenu.children[0].height < id_loadMenuItem.item.m_iH )
                {
                    id_loadMenuItem.height = height - id_colMenu.children[0].height;
                }
                else
                {
